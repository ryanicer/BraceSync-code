...
// 实现一起错，故这里打真库）。
//
// 🔴 T290-F 已登记（不在本卡修）：feeling_logs.comfort_level 是 VARCHAR(8)（000015:9），
// 而枚举值 'discomfort' 有 10 字符 ⇒ PG14 实测写「不适」直接 ERROR: value too long，
// 000015 自带的 comfort_score<3.0 回填 UPDATE 同样报错（本地按序跑 000001..000021 复现）。
// 因此本用例的样本只用 fitted 与 NULL 两态；筛掉 NULL 恰好证明 feeling 是
// comfort_level 列比对，而不是 comfort_score 派生。
package repo
...
// seedT290Logs 两条独立患者 + 三条日志（共享种子库，测后清场，口径同 T248 IT）。
// A：09-05 fitted、09-03 未评（NULL）；B：09-04 fitted。
...
func TestITListFeelingLogsAdmin_T290(t *testing.T) {
	...
	// 全量（keyword 命中两条患者的共同姓名前缀）：三条都在，且每行带出患者姓名
	...
	// feeling=fitted 命中 2 条，未评（comfort_level IS NULL）那条被筛掉
	...
	// 分页切片：pageSize=2 时第二页只剩 1 条，total 仍是全量
	rows, total, err = itStore.ListFeelingLogsAdmin(ctx, FeelingLogAdminFilter{
		Keyword: "T290", Page: 2, PageSize: 2,
	})
	require.NoError(t, err)
	assert.Equal(t, int64(3), total, "total 不受分页影响")
	require.Len(t, rows, 1)
	assert.Equal(t, "2026-09-03", rows[0].LogDate.Format("2006-01-02"))
}
