...
export default defineConfig(({ mode }) => {
  ...
  // 优先级：.env.local > 目标源文件（仅构建模式）> 开发模式默认值
  const apiBaseUrl = envLocal.VITE_API_BASE_URL ?? envTarget.VITE_API_BASE_URL ?? ''
  const useMock = (envLocal.VITE_USE_MOCK ?? envTarget.VITE_USE_MOCK ?? (isProd ? 'false' : 'true')) !== 'false'
