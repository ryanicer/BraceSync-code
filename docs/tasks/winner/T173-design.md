...
| # | 约束 | 证据 | 影响 |
|---|---|---|---|
| 1 | **1 install : 1 baseline，二次校准硬 409** | `000002_p0_fixes.up.sql:23-26` `uk_install_baseline UNIQUE(baseline_id)` + `repo.go:438-440` `ErrConflict` | **今天无法重校准。** 而 `sensor_drift` 告警规则的存在（`engine.go:97-124`）本身就证明零点会漂 —— 一个"漂移是已知故障模式"的系统却禁止重设零点，是设计不自洽。**→ 已裁（2026-09-13，Boss，T174 规矩A）**：此矛盾经裁定**转为有意设计**——校准只做一次且为唯一权威，漂移不复校；本条不再是风险项，约束语义见 §3.6。 |
...
> **✅ D2 已裁（2026-09-13，Boss，记录于 T173-decision.md）**：**批准 D2-3、砍 D2-1**——只认首版基线、无重校准；并由 T174 立规矩A 升格为产品规则（§3.6）：这是**有意设计，不是欠账**。
...
### 3.6 设计约束：单一权威校准（T174 · 规矩A，2026-09-13 Boss 裁定）

**产品规则**：校准只在**出厂 / 技师安装**时做**一次**，且为**唯一权威校准**。后续即便传感器漂移，**现场不复校**——非权威校准会污染数据有效性，数据有效性优先于现场便利。

由本规则派生的实现约束（写侧 T084 `POST /api/v1/baselines` 及一切前端/客户端必须遵守）：

1. `uk_install_baseline` 唯一约束**保留为有意设计，不是 bug**；一 install 多次写 baseline 撞唯一键返回 **409 是正确行为**，不得"修复"。
2. 前端对 409 须友好提示（如「该校准已存在，如需变更请联系出厂」），不得崩溃、不得暗示可现场复校、不得提供任何重试绕行。
3. **明确排除**：不实现任何复校通道 / 复校端点 / 多版本 baseline（D2-1 已砍）；**不碰 `sensor_drift` 告警**（现有告警维持原样，漂移属运维处置域，不等于授权现场重设零点）。
4. 若未来产品决定「允许出厂级复校」，那是新需求，须另立卡重新裁决，不在本约束覆盖范围。
...
| **3** | 重校准语义（已裁：规矩A，§3.6）+ 技师端 placebo 收口（按 D4） | 二次校准**保持 409**（有意设计，非欠账）；placebo 按 D4 裁决处理 |
...
> **✅ 裁决落档（2026-09-13，Boss，全文见 `T173-decision.md`）**：**D2 = 批准 D2-3、砍 D2-1**，并经 T174 立规矩A 升格为产品规则（§3.6：单一权威校准、禁止现场复校）——"二次校准永远 409"由此转为**有意设计**，上表"不裁的后果"列不再适用。D1–D8 其余各项的裁决结论同样以 `T173-decision.md` 为准。
