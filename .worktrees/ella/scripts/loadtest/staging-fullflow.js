...
//
// ===== Scenarios =====
// - loginBurst:     0鈫?00 VUs over 5s, hold 10s      (validate auth throughput)
// - deviceIngest:   0鈫?0 VUs over 10s, hold 60s      (signature verification + DB insertion)
// - dashboardQuery: 0鈫?0 VUs over 5s, hold 30s       (aggregation queries P95鈮?00ms)
//
// ===== Thresholds =====
// - http_req_duration: p(95)<400ms (P95 鈮?400ms)
...
// - P95 鈮?400ms target from runbook 搂9
...
// ===== Configuration (ENV) =====
const TARGET_URL = __ENV.TARGET_URL || 'http://localhost:2080'; // SSH tunnel 鈫?81
...
const DEVICE_SECRET = __ENV.DEVICE_SECRET || ''; // 鈿狅笍 Required for device-report; empty = SKIP
...
export const options = {
  ...
  thresholds: {
    'http_req_duration': ['p(95)<400'], // Target: P95 鈮?400ms
    'http_req_failed': ['rate<0.05'], // <5% error rate
  },
};
...
function queryAlerts(token) {
  // 鍚庣濂戠害锛欸ET /api/v1/alerts?page=1&pageSize=20
  // 杩斿洖缁撴瀯锛歿 code: 200, message: "success", data: { list: [], total: 6, page: 1, pageSize: 20 } }
...
function queryDashboard(token) {
  // 鍚庣濂戠害锛欸ET /api/v1/admin/dashboard/kpi?period=today|week|month
  // 杩斿洖缁撴瀯锛歿 code: 200, message: "success", data: { totalPatients, todayActiveWear, todayAlerts, avgWearHours, deviceOnlineRate, monthNewPatients } }
  ...
  if (data.thresholds) {
    if (data.thresholds['http_req_duration']) {
      console.log('http_req_duration(p95<400ms):', 
        data.thresholds['http_req_duration'].passed ? '鉁?PASS' : '鉂?FAIL');
    }
    if (data.thresholds['http_req_failed']) {
      console.log('http_req_failed(rate<5%):', 
        data.thresholds['http_req_failed'].passed ? '鉁?PASS' : '鉂?FAIL');
    }
  }
...
function generateTextSummary(data) {
  ...
  if (data.metrics.checks) {
    ...
    summary += `  鉁?${checks.values.passes} / ${checks.values.passes + checks.values.fails} - ${((checks.values.passes / (checks.values.passes + checks.values.fails)) * 100).toFixed(2)}%\n`;
  }
...
export function handleRunComplete(testRunResult) {
  console.log('\n[INFO] 鍘嬫祴瀹屾垚锛岃妫€鏌?./reports/鐩綍鐢熸垚鐨勬姤鍛婂拰缁熻鏁版嵁');
}
