...
test.describe('14-角色页面准入与库同源（T368）', () => {
  test('14.1 doctor_li：侧栏页数 = /me/permissions 项数 = 6（补 review / review_tpl）', async ({ page }) => {
    ...
    await requireDeployedBuild(page, {
      marker: 'T368-doctor-review-modules',
      why: 'staging 库仍跑 000017 播的 4 项，迁移 000028 未前滚 ⇒ /me/permissions 只回 4，本条判据无意义',
      probe: async () => (await readMePermissions(page)).modules.length === 6,
    })
    ...
    expect(perms.modules).toEqual(['dashboard', 'realtime', 'alerts', 'orthosis', 'review', 'review_tpl'])
    expect(sidebar, `侧栏 ${sidebar.length} 项 vs 接口 ${perms.modules.length} 项 = 又分叉了`).toHaveLength(6)
  ...
  test('14.2 ops_admin：侧栏页数 = /me/permissions 项数 = 15（不退化）', async ({ page }) => {
    await realLogin(page)
    const sidebar = await sidebarTitles(page)
    const perms = await readMePermissions(page)
    expect(perms.roleId).toBe('ROLE_ADMIN')
    expect(perms.modules).toHaveLength(15)
